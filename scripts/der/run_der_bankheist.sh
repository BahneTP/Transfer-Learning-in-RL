...
RUN_ROOT="${RUN_ROOT:-logs/der/der_bankheist_${STAMP}}"
...
for seed in ${SEEDS}; do
  ...
  RUN_NAME="der_bankheist_seed_${seed}_${STAMP}"

  echo "Starting DER BankHeist seed ${seed} on GPU ${GPU}"
  CUDA_VISIBLE_DEVICES="${GPU}" uv run python src/train.py \
    experiment=atari100k/der/bankheist \
    algorithm.encoder_type=dqn \
    algorithm.transfer_mode=none \
    trainer.seed="${seed}" \
    trainer.devices="[0]" \
