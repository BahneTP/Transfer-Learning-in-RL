...
RUN_ROOT="${RUN_ROOT:-logs/resnet/resnet_attentive_jamesbond_${STAMP}}"
...
for seed in ${SEEDS}; do
  ...
  RUN_NAME="resnet_attentive_jamesbond_seed_${seed}_${STAMP}"

  echo "Starting seed ${seed} on GPU ${GPU}"
  CUDA_VISIBLE_DEVICES="${GPU}" uv run python src/train.py \
    experiment=resnet/der/attentive/jamesbond \
    trainer.seed="${seed}" \
    trainer.devices="[0]" \
