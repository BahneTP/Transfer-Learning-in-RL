...
RUN_ROOT="${RUN_ROOT:-logs/resnet/resnet_lora_jamesbond_rank32_alpha64_${STAMP}}"
...
for seed in ${SEEDS}; do
  ...
  RUN_NAME="resnet_lora_jamesbond_r32_a64_seed_${seed}_${STAMP}"
  ...
  CUDA_VISIBLE_DEVICES="${GPU}" uv run python src/train.py \
    ...
    algorithm.lora_rank=32 \
    algorithm.lora_alpha=64.0 \
    2>&1 | tee "${LOG_FILE}"
  status="${PIPESTATUS[0]}"
