...
RUN_ROOT="${RUN_ROOT:-logs/resnet/resnet_jamesbond_linear_layer3_${STAMP}}"
...
for seed in ${SEEDS}; do
  ...
  RUN_NAME="resnet_jamesbond_linear_layer3_seed_${seed}_${STAMP}"

  echo "Starting seed ${seed} on GPU ${GPU}"
  CUDA_VISIBLE_DEVICES="${GPU}" uv run python src/train.py \
    experiment=resnet/der/linear_layer3/jamesbond \
    trainer.seed="${seed}" \
    trainer.devices="[0]" \
