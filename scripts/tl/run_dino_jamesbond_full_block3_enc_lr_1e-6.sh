...
RUN_ROOT="${RUN_ROOT:-logs/tl/dino_jamesbond_full_block3_enc_lr_1e-6_${STAMP}}"
...
for seed in ${SEEDS}; do
  ...
  RUN_NAME="dino_jamesbond_full_block3_enc_lr_1e-6_seed_${seed}_${STAMP}"

  echo "Starting seed ${seed} on GPU ${GPU}"
  CUDA_VISIBLE_DEVICES="${GPU}" uv run python src/train.py \
    experiment=dinov2/der/full_block3/jamesbond \
    ...
    algorithm.encoder_lr_scale=0.01 \
    2>&1 | tee "${LOG_FILE}"
  status="${PIPESTATUS[0]}"
