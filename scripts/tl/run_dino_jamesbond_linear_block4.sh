...
RUN_ROOT="${RUN_ROOT:-logs/tl/dino_jamesbond_linear_block4_${STAMP}}"
...
for seed in ${SEEDS}; do
  ...
  RUN_NAME="dino_jamesbond_linear_block4_seed_${seed}_${STAMP}"

  echo "Starting seed ${seed} on GPU ${GPU}"
  CUDA_VISIBLE_DEVICES="${GPU}" uv run python src/train.py \
    experiment=dinov2/der/linear_block4/jamesbond \
