...
RUN_ROOT="${RUN_ROOT:-logs/tl/dino_jamesbond_jepa_block3_weight_0p01_${STAMP}}"
...
for seed in ${SEEDS}; do
  ...
  RUN_NAME="dino_jamesbond_jepa_block3_weight_0p01_seed_${seed}_${STAMP}"
  ...
  CUDA_VISIBLE_DEVICES="${GPU}" uv run python src/train.py \
    ...
    algorithm.jepa_loss_weight=0.01 \
    2>&1 | tee "${LOG_FILE}"
  status="${PIPESTATUS[0]}"
