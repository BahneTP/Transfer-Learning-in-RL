...
RUN_ROOT="${RUN_ROOT:-logs/tl/dino_jamesbond_jepa_block3_weight_0p1_direct_history3_${STAMP}}"
...
for seed in ${SEEDS}; do
  ...
  RUN_NAME="dino_jamesbond_jepa_block3_weight_0p1_direct_history3_seed_${seed}_${STAMP}"

  echo "Starting seed ${seed} on GPU ${GPU}"
  CUDA_VISIBLE_DEVICES="${GPU}" uv run python src/train.py \
    experiment=dinov2/der/jepa_block3/jamesbond \
    algorithm.dinov2_weights="${WEIGHTS}" \
    algorithm.jepa_loss_weight=0.1 \
    algorithm.jepa_prediction_mode=direct \
    +algorithm.stack_size=3 \
    trainer.seed="${seed}" \
    trainer.devices="[0]" \
