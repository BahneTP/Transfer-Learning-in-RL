...
RUN_ROOT="${RUN_ROOT:-logs/tl/dino_jamesbond_lora_block3_straight_0p1_${STAMP}}"
...
for seed in ${SEEDS}; do
  ...
  RUN_NAME="dino_jamesbond_lora_block3_straight_0p1_seed_${seed}_${STAMP}"

  echo "Starting seed ${seed} on GPU ${GPU}"
  CUDA_VISIBLE_DEVICES="${GPU}" uv run python src/train.py \
    experiment=dinov2/der/lora_block3/jamesbond \
    algorithm.dinov2_weights="${WEIGHTS}" \
    algorithm.temporal_straightening_weight=0.1 \
